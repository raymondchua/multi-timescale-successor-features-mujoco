...
@SUITE.add("benchmarking")
def walk(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, mass_scale=1.0, friction_scale=1.0, forward_scale=1.0, use_forward_scale=False):
    """Returns the Walk task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = Humanoid(move_speed=_WALK_SPEED, pure_state=False, random=random, mass_scale=mass_scale, friction_scale=friction_scale, forward_scale=forward_scale, use_forward_scale=use_forward_scale)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
def run(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, mass_scale=1.0, friction_scale=1.0, forward_scale=1.0, use_forward_scale=False):
    ...
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = Humanoid(move_speed=_RUN_SPEED, pure_state=False, random=random, mass_scale=mass_scale, friction_scale=friction_scale, forward_scale=forward_scale, use_forward_scale=use_forward_scale)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
def run_backward(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, mass_scale=1.0, friction_scale=1.0):
    """Returns the Run task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = Humanoid(move_speed=_RUN_SPEED, pure_state=False, random=random, forward=False, mass_scale=mass_scale, friction_scale=friction_scale)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
def modify_humanoid_dynamics(physics, mass_scale=1.0, friction_scale=1.0):
...
class Humanoid(base.Task):
    ...
    def __init__(self, move_speed, pure_state, random=None, forward=True, mass_scale=1.0, friction_scale=1.0, forward_scale=1.0, use_forward_scale=False):
    ...
    def initialize_episode(self, physics):
        ...
        penetrating = True
        modify_humanoid_dynamics(physics, mass_scale=self._mass_scale, friction_scale=self._friction_scale)
        while penetrating:
    ...
    def get_reward(self, physics):
        ...
        if self._move_speed == 0:
            ...
            move = rewards.tolerance(
                self._forward_scale * com_velocity,
