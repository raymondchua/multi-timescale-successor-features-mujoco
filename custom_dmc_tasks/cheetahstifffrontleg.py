...
def get_model_and_assets():
    ...
    xml = resources.GetResource(
        os.path.join(root_dir, "custom_dmc_tasks", "cheetahstifffrontleg.xml")
    )
...
def run(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    ...
    task = CheetahStiffFrontLeg(random=random)
...
def run_backward(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    """Returns the run task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = CheetahStiffFrontLeg(forward=False, flip=False, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics, task, time_limit=time_limit, **environment_kwargs
    )
...
def flip(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    """Returns the run task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = CheetahStiffFrontLeg(forward=True, flip=True, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics, task, time_limit=time_limit, **environment_kwargs
    )
...
def flip_backward(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    """Returns the run task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = CheetahStiffFrontLeg(forward=False, flip=True, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics, task, time_limit=time_limit, **environment_kwargs
    )
...
class Physics(mujoco.Physics):
    """Physics simulation with additional features for the CheetahStiffFrontLeg domain."""

    def speed(self):
        """Returns the horizontal speed of the CheetahStiffFrontLeg."""
        return self.named.data.sensordata["torso_subtreelinvel"][0]

    def angmomentum(self):
        """Returns the angular momentum of torso of the CheetahStiffFrontLeg about Y axis."""
        return self.named.data.subtree_angmom["torso"][1]


class CheetahStiffFrontLeg(base.Task):
    """A `Task` to train a running CheetahStiffFrontLeg."""

    def __init__(self, forward=True, flip=False, random=None):
        self._forward = 1 if forward else -1
        self._flip = flip
        super(CheetahStiffFrontLeg, self).__init__(random=random)
