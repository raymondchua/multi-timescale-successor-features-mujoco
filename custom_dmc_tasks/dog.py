...
def walk_backward(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    ...
    floor_size = move_speed * _DEFAULT_TIME_LIMIT
    physics = Physics.from_xml_string(*get_model_and_assets(floor_size))
    task = Move(move_speed=move_speed, random=random, forward=False)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
class Physics(mujoco.Physics):
    ...
    def z_projection(self):
        ...
        return np.vstack(
            (
                self.named.data.xmat["skull", ["zx", "zy", "zz"]],
    ...
    def ball_in_head_frame(self):
        ...
        return np.hstack(
            (head_to_ball.dot(head_frame), head_to_ball_vel.dot(head_frame))
        )
...
class Stand(base.Task):
    ...
    def get_reward_factors(self, physics):
        ...
        torso = rewards.tolerance(
            physics.torso_pelvis_height()[0],
            bounds=(self._stand_height[0], float("inf")),
            margin=self._stand_height[0],
        )
        # Keep the pelvis at standing height.
        pelvis = rewards.tolerance(
            physics.torso_pelvis_height()[1],
        ...
        touch = rewards.tolerance(
            physics.touch_sensors().sum(),
...
class Move(Stand):
    ...
    def __init__(self, move_speed, random, observe_reward_factors=False, forward=True):
    ...
    def get_reward_factors(self, physics):
        ...
        forward = rewards.tolerance(
            self._forward * physics.com_forward_velocity(),
...
class Fetch(Stand):
    ...
    def initialize_episode(self, physics):
        ...
        direction = np.array(
            (
                -np.sin(azimuth) + 0.05 * self.random.randn(),
