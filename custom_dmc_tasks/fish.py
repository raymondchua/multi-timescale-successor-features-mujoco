...
@SUITE.add("benchmarking")
def swim(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, mass_scale=1.0, friction_scale=1.0):
    """Returns the Fish Swim task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = Swim(random=random, mass_scale=mass_scale, friction_scale=friction_scale)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
@SUITE.add("benchmarking")
def swim_backward(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, mass_scale=1.0, friction_scale=1.0):
    """Returns the Fish Swim backward task. Essentially the fish has to swim away from the target."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = Swim(random=random, forward=False, mass_scale=mass_scale, friction_scale=friction_scale)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
def modify_fish_dynamics(physics, mass_scale=1.0, friction_scale=1.0):
...
class Swim(base.Task):
    ...
    def __init__(self, random=None, forward=True, mass_scale=1.0, friction_scale=1.0):
    ...
    def initialize_episode(self, physics):
        ...
        modify_fish_dynamics(physics, mass_scale=self._mass_scale, friction_scale=self._friction_scale)
        super().initialize_episode(physics)
    ...
    def get_reward(self, physics):
        ...
        in_target = rewards.tolerance(
            self._forward * np.linalg.norm(physics.mouth_to_target()),
            bounds=(0, radii),
            margin=2 * radii,
        )
