...
def modify_quadruped_dynamics(physics, mass_scale=1.0, friction_scale=1.0):
    """Scales body mass and geom sliding friction in the quadruped environment."""
...
@SUITE.add()
def roll(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, mass_scale=1.0, friction_scale=1.0, forward_scale=1.0, use_forward_scale=False):
    """Returns the Walk task."""
    xml_string = make_model(floor_size=_DEFAULT_TIME_LIMIT * _WALK_SPEED)
    physics = Physics.from_xml_string(xml_string, common.ASSETS)
    task = Roll(desired_speed=_WALK_SPEED, random=random, mass_scale=mass_scale, friction_scale=friction_scale, forward_scale=forward_scale, use_forward_scale=use_forward_scale)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
@SUITE.add()
def roll_backwards_fast(
    time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None
):
    """Returns the Walk task."""
    xml_string = make_model(floor_size=_DEFAULT_TIME_LIMIT * _WALK_SPEED)
    physics = Physics.from_xml_string(xml_string, common.ASSETS)
    task = Roll_Backwards(desired_speed=_RUN_SPEED, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
def run(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, mass_scale=1.0, friction_scale=1.0, forward_scale=1.0, use_forward_scale=False):
    ...
    physics = Physics.from_xml_string(xml_string, common.ASSETS)
    task = Move(desired_speed=_RUN_SPEED, random=random, mass_scale=mass_scale, friction_scale=friction_scale, forward_scale=forward_scale, use_forward_scale=use_forward_scale)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
def run_backward(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None, mass_scale=1.0, friction_scale=1.0):
    ...
    xml_string = make_model(floor_size=_DEFAULT_TIME_LIMIT * _RUN_SPEED)
    physics = Physics.from_xml_string(xml_string, common.ASSETS)
    task = Move_Backwards(desired_speed=_RUN_SPEED, random=random, mass_scale=mass_scale, friction_scale=friction_scale)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics,
        task,
...
class Move(base.Task):
    ...
    def __init__(self, desired_speed, random=None, mass_scale=1.0, friction_scale=1.0, forward_scale=1.0, use_forward_scale=False):
    ...
    def initialize_episode(self, physics):
        ...
        modify_quadruped_dynamics(
            physics, mass_scale=self._mass_scale, friction_scale=self._friction_scale
        )
    ...
    def get_reward(self, physics):
        ...
        # Move reward term.
        if self._use_forward_scale:
            move_reward = rewards.tolerance(
                self._forward_scale * physics.torso_velocity()[0],
...
class Move_Backwards(base.Task):
    """A quadruped task solved by moving backwards at a designated speed."""

    def __init__(self, desired_speed, random=None, mass_scale=1.0, friction_scale=1.0):
        """Initializes an instance of `Move`.
        Args:
          desired_speed: A float. If this value is zero, reward is given simply
            for standing upright. Otherwise this specifies the horizontal velocity
    ...
    def get_reward(self, physics):
        ...
        # Move reward term.
        move_reward = rewards.tolerance(
            -physics.torso_velocity()[0],  # negative to move backwards
            bounds=(self._desired_speed, float("inf")),
            margin=self._desired_speed,
            value_at_margin=0.5,
            sigmoid="linear",
...
class Roll(base.Task):
    ...
    def get_reward(self, physics):
        ...
        if self._use_forward_scale:
            move_reward = rewards.tolerance(
                self._forward_scale * np.linalg.norm(physics.torso_velocity()),
                bounds=(self._desired_speed, float("inf")),
                margin=self._desired_speed,
                value_at_margin=0.5,
                sigmoid="linear",
...
class Roll_Backwards(base.Task):
    """A quadruped task solved by moving backwards at a designated speed."""
    ...
    def get_reward(self, physics):
        ...
        # Move reward term.
        move_reward = rewards.tolerance(
            -1 * np.linalg.norm(physics.torso_velocity()),  # negative to move backwards
            bounds=(self._desired_speed, float("inf")),
            margin=self._desired_speed,
            value_at_margin=0.5,
            sigmoid="linear",
