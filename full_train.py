...
from coverage_logger import CoverageLoggerCSV
...
class Workspace:
    def __init__(self, cfg):
        ...
        # create envs
        if self.cfg.primal_tasks:
            logging.info("Using primal tasks")
            self.tasks = [PRIMAL_TASKS[self.cfg.domain]]
            self.cfg.terminate_after_first_task = True

        elif self.cfg.primal_tasks_walk:
            logging.info("Using primal tasks walk")
        ...
        # different domains and different reward function (eg. cheetah-run and quadruped-jump)
            ...
            logging.info("Different domains with different reward function for all tasks")
        ...
        if self.cfg.track_states:
            self.coverage_logger = CoverageLoggerCSV(
                output_dir = Path(self.cfg.snapshot_dir) / "coverage_snapshots"
            )
        ...
        # grab the max number of actions based on the domain
        for i in range(self.num_tasks):
            name = self.tasks[i]
            domain, task = name.split("_", 1)
            self.num_actions.append(num_actions[domain])

        max_action_shape = np.max(self.num_actions)
        print(f"max_action_shape: {max_action_shape}")
        ...
        for i in range(self.num_tasks):
            ...
            train_env = dmc.make(
                ...
                self.cfg.seed,
                num_actions=max_action_shape,
                num_valid_actions=self.num_actions[i],
            )
            eval_env = dmc.make(
                task,
                self.cfg.obs_type,
            ...
            self.obs_specs.append(train_env.observation_spec())
        ...
        max_obs_shape = np.max([x.shape for x in self.obs_specs], axis=0)

        # wrap the envs to pad the observations to the max shape with zeros
        for i in range(self.num_tasks):
            self.train_envs[i] = dmc.ObservationPadWrapper(self.train_envs[i], max_obs_shape)
            self.eval_envs[i] = dmc.ObservationPadWrapper(self.eval_envs[i], max_obs_shape)
        ...
        action_spec = specs.Array(
            shape=np.array([max_action_shape]),
            dtype=np.float32,
            name="action",
        )
    ...
    def eval(self, num_valid_actions: int, task_id: int = None, meta=None):
    ...
    def train(self):
        ...
        if self.agent.consolidation:
            self.agent.num_train_frames = self.cfg.num_train_frames // self.cfg.action_repeat
            self.agent.set_up_consolidation_system()

        save_coverage_frequency = self.cfg.num_train_frames // self.cfg.save_coverage_frequency_factor
        ...
        for exposure_id in range(self.cfg.num_exposures):
            ...
            for task_id in range(self.num_tasks):
                ...
                num_valid_actions = self.num_actions[task_id]
                ...
                while train_until_step(task_step + 1):
                    ...
                    if eval_every_step(self.global_step):
                        ...
                        self.eval(num_valid_actions, task_id, meta)
                    ...
                    if self.cfg.track_states:
                        x, y = current_train_env.physics.data.qpos[:2]
                        ...
                        if self.global_step % save_coverage_frequency == 0:
                            self.coverage_logger.save(self.global_step)
